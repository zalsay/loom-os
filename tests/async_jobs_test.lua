...
jobs.set_capability_adapter({
    call = function(name, args)
        ...
        if name == "lua_run_script_async" then
            return "Started Lua job job-42 (name=ota, exclusive=clawos-update, timeout_ms=0, log_bytes=4096, status=running) for /fatfs/worker.lua. Use lua_get_async_job or lua_tail_async_job with job_id=job-42 to read logs/results."
        end
        return "ok"
    end,
})
