...
jobs.set_capability_adapter({
    call = function(name, payload)
        if name == "lua_run_script_async" then
            ...
            assert(payload.exclusive == "clawos-runtime-update")
            return "Started Lua job test-job (name=clawos-runtime-update, exclusive=clawos-runtime-update, timeout_ms=0, log_bytes=8192, status=running) for " .. payload.path .. ". Use lua_get_async_job or lua_tail_async_job with job_id=test-job to read logs/results."
        end
        return "ok"
    end,
})
...
local started = assert(jobs.start({
    path = "/fatfs/clawos-runtime/releases/0.1.0/update/async_install_worker.lua",
    args = { request_path = "/fatfs/clawos-runtime/jobs/runtime-update-0.1.1.json" },
    timeout_ms = 0,
    log_bytes = 8192,
