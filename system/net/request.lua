-- Nonblocking request jobs. A verified device adapter implements start/poll/cancel.
...
function M.set_adapter(value)
    if value ~= nil and (type(value.start) ~= "function" or type(value.poll) ~= "function") then
        return nil, errors.new("E_INVALID_ARG", "network adapter needs start and poll")
    end
    adapter = value
    return true
end
function M.create(options, callback)
    if not adapter then return nil, errors.new("E_UNSUPPORTED", "nonblocking network adapter unavailable") end
    if type(options) ~= "table" or type(options.url) ~= "string" or options.url:sub(1, 8) ~= "https://" then
        return nil, errors.new("E_INVALID_ARG", "HTTPS URL required")
    end
